...
- Browser pages: `/`, `/config`, `/zotero`, `/literature`, `/ontology`, `/curate-prompts`, `/ontology-suggestions`, `/curation`, `/export`; `/curation-prompt` remains a compatibility route to the static app but is no longer shown as a separate prompt section.
...
- Ontology Suggestions UI is intentionally compact; richer in-page review/edit cards and budget dashboards can be expanded later. Curate Prompts now supports core template editing/versioning plus duplicate/archive controls, while accepted/edited suggestions already enter the existing candidate queue through review.
