...
- Ontology Suggestions: `/api/ontology-suggestions/status`, Markdown/front-matter prompt-template load/save/duplicate/deactivate, preview, provider connectivity diagnostics, real LLM pipeline test diagnostics, real suggestion run, pricing, run history, and review endpoints. This workflow reads selected project-linked staged/curated literature and the project ontology, stores run files under `data/ontology_suggestions`, validates strict JSON before persisting suggestions, and creates normal candidate-queue entries only after accepted/edited human review.
...
- Ontology Suggestions UI is intentionally compact; richer in-page review/edit cards and budget dashboards can be expanded later. Prompts now supports core template editing/versioning plus duplicate/archive controls, while accepted/edited suggestions already enter the existing candidate queue through review.
