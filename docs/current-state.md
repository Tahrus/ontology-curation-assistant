...
- Browser pages: `/`, `/config`, `/zotero`, `/literature`, `/ontology`, `/prompts`, `/ontology-suggestions`, `/curation`, `/export`; `/curation-prompt` remains a compatibility route to the static app but is no longer shown as a separate prompt section.
...
- Browser UI includes client-side route handling for dashboard/header links, a fixed top bar, a persistent active-project banner, project hierarchy dashboard, page-scoped startup data loading, a visible startup/page error path, accessible button/link click feedback, long-running action busy states, a Light/Dark theme toggle persisted in local storage, and a smaller shared logo link back to the dashboard.
- Browser Configuration includes guarded Zotero configuration/test controls, while Literature owns Zotero import preview/import actions with compact visible summaries and collapsed per-item detail reports for imported, already-present, wrong-collection, no-PDF, no-identifier, unsupported-publisher, manual-Markdown-required, and failed Zotero records, provider/model dropdowns for Gemini, OpenAI, Anthropic, and custom OpenAI-compatible LLMs, preset defaults for model/base URL/env var/runtime settings, API key environment variable support, a `Test LLM connection` action with key-source and canonical provider-key diagnostics, publisher/source API settings for Elsevier, Springer, Wiley, Crossref, NCBI, and OpenAlex with masked status diagnostics, and Docker/ODK diagnostics for mounted paths and tools. The Literature page separates `Curated Literature` from `New / Uncurated Literature` in mutually exclusive sub-tabs; both views support search/status filtering, canonical project-tag filtering, provenance/status display, and review/edit actions.
- Configuration: `/api/config/status`, `/api/config/zotero`, `/api/config/llm`, `/api/config/ontology-path`, `/api/config/test-zotero`.
- Two-stage literature APIs cover staged/curated listing, staged review/edit/promote/reject, curated edit/project tags, publisher-API-required identified import, explicitly authorized PDF modes, import diagnostics, isolated publisher API testing, dry-run/confirmed unpromoted cleanup with legacy managed-orphan repair, and publisher settings. Import remains `/api/literature/import`; Zotero import and sync responses now return structured per-item result lists with title, Zotero key, DOI, status, and reason; diagnostics are `/api/literature/import-diagnostics` and `/api/literature/test-publisher-api`; `/api/literature/pipeline/run` remains a compatibility alias that obeys the configured mode.
...
- Ontology Suggestions: `/api/ontology-suggestions/status`, Markdown/front-matter prompt-template load/save/duplicate/deactivate, preview, provider connectivity diagnostics, real LLM pipeline ontology-draft diagnostics, real draft run, pricing, run history, and review endpoints. This workflow sends task prompt + optional selected Markdown literature + optional existing OBO context through the configured LLM provider, validates draft JSON (`ontology_draft`, `proposed_terms`, `proposed_relations`, `warnings`), logs raw/parsed diagnostics under `data/ontology_suggestions`, and does not automatically merge or persist draft terms as accepted ontology changes.
...
- `oca suggestions prompts/test/test-api/run/list-runs/show-run` manage the read-only ontology-suggestion workflow, including cheap dummy tests and real single-paper runs stored outside the literature pipeline.
...
- `oca llm-test` tests the configured LLM provider with a small Markdown literature entry, synthetic OBO ontology, and JSON extraction/mapping prompt, then reports provider, canonical provider key, model, key presence/source, latency, raw response preview, parsed JSON when available, whether the Markdown content check passed, and whether the ontology mapping check passed.
...
- Browser project-management scaffolding for creating/editing/selecting projects through a four-step wizard, project cards, a selected-project detail/next-step panel, parent/child project hierarchy, existing ontologies as normal `existing_ontology_project` nodes, base IRI suggestions until manual override, optional configured-LLM metadata drafts, preserved field values on failed saves, structured project errors, and optional path statuses as warnings rather than hard failures. Project-tag aliases such as a project name slug are resolved to the single canonical project tag before display/filtering.
...
- Ontology Suggestions UI is intentionally compact; richer in-page draft review/edit cards and budget dashboards can be expanded later. Prompts now supports core template editing/versioning plus duplicate/archive controls. The first LLM ontology workflow step currently keeps generated ontology drafts as preview/raw run artifacts rather than merging them into ontology files.
