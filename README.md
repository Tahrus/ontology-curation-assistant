...
- Settings: Zotero metadata/API settings, LLM provider/model settings, LLM connection testing, Docker/ODK diagnostics, Zotero connection testing, and the local Zotero literature source path used by explicit PDF/debug modes.
...
- Ontology Suggestions: read-only project-scoped LLM ontology-draft generation over optional selected literature and optional existing OBO context, with prompt template selection, rich diagnostics, token/cost estimates, run logs, parsed JSON previews, and no automatic ontology merge.
...
- test Zotero credentials, import top-level Zotero literature records from the Literature page, load local test Zotero entries, or run explicit PDF/debug imports when configured
...
- run read-only ontology suggestion diagnostics and single/multi-paper ontology draft analyses without modifying literature, ontology files, ODK templates, GitHub state, or project tags
...
Use Zotero Connection to test the credentials, then open Literature and click `Preview Zotero Import` to see every Zotero item found before filtering, including Zotero key, title, item type, DOI, tags, collections, attachment counts, PDF detection, and the planned import/skip reason. Click `Import from Zotero` on Literature to import top-level Zotero metadata records through the publisher/API-required workflow. The browser sync defaults to no limit and follows Zotero pagination until all records in the configured library or collection are retrieved. An advanced optional limit is available only for testing. Metadata-only entries, including `manual_markdown_required` records, remain visible in the Literature review list. Literature import keeps a compact summary visible and places the detailed per-item report in a collapsed `Show detailed import report` section.
The Zotero configuration/test controls and Literature import controls are bound after the DOM is ready and use stable field IDs. If a required panel element is missing, the UI shows a reload message instead of throwing a null-selector error.
...
Use `Test LLM connection` in the UI or `oca llm-test` from the CLI. The test sends a small Markdown literature entry, a synthetic OBO ontology, and a JSON extraction/mapping prompt through the currently configured provider/model/API key, then reports provider, canonical provider key, model, API key presence/source, latency, raw response preview, parsed JSON when available, whether the Markdown content check passed (`sample_count.value=42`, `ph.value=7.4`, `unit_operation.value=chromatography`), and whether the ontology mapping check passed (`OCA_TEST:0000003`, `OCA_TEST:0000004`, `OCA_TEST:0000002`). API keys are masked in responses and logs. Provider labels and aliases such as `Gemini API`, `Google Gemini`, `google_gemini`, `google-ai`, and `google_ai` are normalized to the canonical `gemini` provider before saving, activating saved configs, and making real LLM calls.
...
Run read-only ontology draft generation from the browser at <http://127.0.0.1:8000/suggestions> on the Run panel, or run the legacy suggestion CLI commands:
...
Suggestion workflow files are stored separately from the literature pipeline under `data/ontology_suggestions/{prompts,runs,logs}`. Prompt templates live as one Markdown file with YAML front matter per template under `data/ontology_suggestions/prompts/`. The provider connectivity test sends only a minimal strict-JSON request and records diagnostics under `data/ontology_suggestions/logs/api_function_tests/`. The main LLM pipeline test and real browser run now assemble the same controlled request shape: task prompt plus clearly labeled selected Markdown literature plus clearly labeled existing OBO context when available, followed by a JSON-only ontology-draft contract. Literature selection is optional, and missing OBO context is represented explicitly in the prompt. Runs parse direct or fenced/recovered JSON, validate `ontology_draft`, `proposed_terms`, `proposed_relations`, and `warnings`, show raw/parsed/validation diagnostics, log raw responses under `data/ontology_suggestions/runs`, and store valid draft output only as a preview/raw curation-run artifact. Nothing is automatically accepted, merged, or written to ontology files.
